...
from ..services import (MappingService, TheoDoiReader, NovaNhapProcessor, BangKeWriter, PhuPhiService,
                        ParallelSheetWriter, OrderProcessor, FlowRunner, FlowConfig)


class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Auto Fill Bảng Kê – Nova")
    ...
    def _on_process(self):
        if not all([self.mapping_path, self.theo_doi_path, self.bang_ke_path]):
            QMessageBox.warning(self, "Thiếu file em âyy", "Chọn cho đủ file coaiii")
            return
        ...
        try:
            ...
            # CONFIG FLOW
            # ===============================
            NHAP_CONFIG = FlowConfig(
                name="NHẬP",
                theo_doi_sheet="NOVA NHẬP",
                mapping_sheet="Nova Nhập",
                phu_phi_sheet="Phụ Phí Nhập",
                parallel_sheet="nhập"
            )

            XUAT_CONFIG = FlowConfig(
                name="XUẤT",
                theo_doi_sheet="NOVA XUẤT",
                mapping_sheet="Nova Xuất",
                phu_phi_sheet="Phụ Phí Xuất",
                parallel_sheet="xuất"
            )
            ...
            current_row = FlowRunner(
                NHAP_CONFIG,
                mapping_service,
                reader,
                writer,
                theo_doi_wb
            ).run(month, start_row=current_row)
            ...
            # ===============================
            # PARALLEL SHEETS (GIỮ NGUYÊN)
            # ===============================
            ParallelSheetWriter(
                writer.wb["nhập"]
            ).write_parallel(
                theo_doi_ws=theo_doi_wb["NOVA NHẬP"],
                orders=reader.read_nova_by_month("NOVA NHẬP", month),
                mapping=mapping_service.load_parallel_mapping("nhập"),
                start_row=6
            )
            ...
            ParallelSheetWriter(
                ...
                orders=reader.read_nova_by_month("NOVA XUẤT", month),
                mapping=mapping_service.load_parallel_mapping("xuất"),
                start_row=6
            )
